...
- **StateTree.** Component/AIComponent StateTree asset hierarchy plus schema-filtered node discovery/readback/native placement/reorder/remove/property patching/transition readback+creation+edit+remove/primitive + common/generic struct/object/class parameter declarations and values/BP-base task/condition/consideration/evaluator node assets+override discovery+placement+root/nested instance edits/compile diagnostics are now shipping in Lite and Professional. Delegate transitions, deeper transition-condition workflows, bindings, and binding-path diagnostics continue through the remaining StateTree slices.
