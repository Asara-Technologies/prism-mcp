...
- **StateTree.** Component/AIComponent StateTree asset hierarchy plus schema-filtered node discovery/readback/native placement/reorder/remove/property patching/transition readback+creation+edit+remove/primitive parameter declarations/BP-base node assets+placement+root instance edits/compile diagnostics are now shipping in Lite and Professional. Delegate transitions, deeper transition-condition workflows, richer parameters, bindings, override discovery, and nested Blueprint instance editing continue through the remaining StateTree slices.
