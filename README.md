...
**Lite: gameplay authoring.** Level actors, Blueprints (full authoring surface), Blueprint live debugging, components, Blackboard, Behavior Tree, and EQS option/test/property authoring, content browser, selection, console, PIE. The surface you live in day to day. [Sold on Fab][fab-product].
...
| **PIE** | Start, stop | ✓ | ✓ |
| **AI behavior authoring** | Blackboards, Behavior Tree shells/bindings/nodes/validation/properties, subtree references, BP-base helpers, and EQS asset shells/classes/options/tests/validation/properties | ✓ | ✓ |
...
- **Behavior Trees / EQS.** EQS graph sync and EQS BP-base helpers. Blackboard schema authoring, Behavior Tree shell/composite/task/decorator/service/validation/subnode-management/property/graph-node/update/subtree-reference/BP-base-helper authoring, and EQS asset-shell/class-discovery/option/test-list/validation/property coverage are now shipped in Lite and Professional.
